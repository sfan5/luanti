...
void QueuedMeshUpdate::checkAir(bool *res, u16 cell_size)
{
	bool all_air = true;
...
namespace {
	// Simple smart-pointer helper to avoid messing up the refcounting
	class UnqueuedMeshUpdate {
		std::unique_ptr<QueuedMeshUpdate> inner;
	public:
		UnqueuedMeshUpdate() : inner(std::make_unique<QueuedMeshUpdate>()) {}
		~UnqueuedMeshUpdate() {
			if (inner)
				inner->dropBlocks();
		}

		QueuedMeshUpdate *operator->() {
			assert(inner);
			return inner.get();
		}
		QueuedMeshUpdate *release() { return inner.release(); }
	};
}
	...
	bool urgent, bool from_neighbor)
{
	...
	*/
	bool all_air;
	q->checkAir(&all_air, mesh_grid.cell_size);
	if (from_neighbor && all_air) {
